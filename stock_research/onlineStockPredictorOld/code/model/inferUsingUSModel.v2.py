...
def arg_setting(): 
    parser = argparse.ArgumentParser(description='Use trained LSTM model to predict stock prices')
    defaultDir = os.path.join(dirPath, '../../results/crypto')
    parser.add_argument(
        '-d',
        '--datadir',
        default=defaultDir,
...
def prepareData(df, dbIds):
    ...
    indices = list(['MSFT', 'AAPL', 'GOOG', '^GSPC', '^IXIC', '^DJI'])
...
def getStartingData(claTrain, scaler, dbIds):
    ...
    query = 'select * from Test where Ticker in (' + ", ".join(dbIds) + ') Order by Datetime desc limit 2000'; 
...
def main():
    ...
    nextMin = (curTime+timedelta(minutes=1)).strftime('%Y-%m-%d %H:%M')
    stockList = list(['"MSFT"', '"AAPL"', '"GOOG"', '"^GSPC"', '"^IXIC"', '"^DJI"']) 
    
    claArgs = join(data_dir, 'programArgs.json')
    ...
    xtest = np.load(join(data_dir, 'results/crypto/x_ts.npy'))
