...
def build_timeseries(mat, num_steps, steps_to_skip, y_col_index, TIME_STEPS):
    # We skip the steps_to_skip so it needs to be catered for in for loop
    # y_col_index is the index of column that would act as output column
    # total number of time-series samples would be len(mat) - TIME_STEPS
    dim_0 = mat.shape[0] - TIME_STEPS
    dim_1 = mat.shape[1]
    x = np.zeros((dim_0, TIME_STEPS, dim_1))
    y = np.zeros((dim_0-steps_to_skip,num_steps))
    #print(y.shape)
    #print(mat[TIME_STEPS+1:TIME_STEPS+1+num_steps, y_col_index].reshape(1,num_steps).shape)
    
    for i in tqdm_notebook(range(dim_0-num_steps-steps_to_skip)):
        x[i] = mat[i:TIME_STEPS+i]
        y[i] = mat[TIME_STEPS+i+steps_to_skip:TIME_STEPS+i+steps_to_skip+num_steps, y_col_index].reshape(num_steps,)
...
def arg_setting(now):
        ...
        return stock_name, pred_len, pred_step, batch_size, timesteps, epochs
...
def main():
    ...
    stock_name, pred_len, pred_step, batch_size, time_steps, epochs = arg_setting(now)
    snapshot_dir = os.path.join(dir_path,'model_runs',stock_name,now)
    print(snapshot_dir)
    os.makedirs(snapshot_dir, exist_ok=True)
    log_file = os.path.join(snapshot_dir,'model_output.log')
    ...
    ### Training data
    # build time-series
    x_t, y_t = build_timeseries(x_train, pred_len, pred_step, pred_col_id, time_steps)
    ...
    ### Validation data
    x_v, y_v = build_timeseries(x_valid, pred_len, pred_step, pred_col_id, time_steps)
    ...
    ### Test data
    x_ts, y_ts = build_timeseries(x_test, pred_len, pred_step, pred_col_id, time_steps)
    ...
    outfile = 'figures/'+now+'_'+stock_name+'_TS_'+str(time_steps)+'_BS_'+str(batch_size)+'_NoOfLookAhead_'+str(pred_step)+'.png'
    plot_real_vs_pred(y_pred_org, y_test_t_org, error, outfile)

    # write model to file 
    model_file = now+'_TS_'+str(time_steps)+'_BS_'+str(batch_size)+'_NoOfLookAhead_'+str(pred_step)+'.model'
