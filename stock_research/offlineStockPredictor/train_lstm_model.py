...
def arg_setting():
        ...
        parser.add_argument(
            ...
            help='Input stock file (string), default=\"data/example_data.csv\"'
        )
        parser.add_argument(
            '-n',
            '--predlen',
        ...
        input_file = str(cmd_args.inputfile)
        ...
        return input_file, pred_len, batch_size, timesteps, logfile, outplot, epochs
         
def main():
    input_file, pred_len, batch_size, time_steps, log_file, out_plot, epochs = arg_setting()
    # we will look at last hour to predict next n value, so time step = 60
    #TIME_STEPS = 30
    #BATCH_SIZE = 100
    #OUTPUT_PATH='log/'
    ...
    ### Training data
    # build time-series
    x_t, y_t = build_timeseries(x_train, pred_len, 3, time_steps)
    # trim extra rows from training data
    x_t = trim_dataset(x_t, batch_size)
    y_t = trim_dataset(y_t, batch_size)

    ### Test data
    x_temp, y_temp = build_timeseries(x_test, pred_len, 3, time_steps)
    ...
    y_test_t_org = (y_test_t_mean * min_max_scaler.data_range_[3]) + min_max_scaler.data_min_[3]
