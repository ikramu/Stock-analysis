...
def arg_setting(now):
        ...
        return stock_name, pred_len, batch_size, timesteps, epochs
...
def main():
    ...
    stock_name, pred_len, batch_size, time_steps, epochs = arg_setting(now)
    ...
    model_file = dir_path + 'models/'+stock_name+'_'+now+'_TS_'+str(time_steps)+'_BS_'+str(batch_size)+'.model'
    # read data
    ...
    outfile = 'figures/'+stock_name+'_TS_'+str(time_steps)+'_BS_'+str(batch_size)+'_Error_'+str(round(error,5))+'.png'
