...
def main():
    marketType = 'se'
    dbInfoFile = os.path.join(dirPath, '../conf/db.yaml')
    ...
    dataDir = os.path.join(basePath, 'data', 'bitcoinData')
    #dataDir = os.path.join(basePath, 'data', 'temp')
    interval = '1m'

    with open(dirPath+'/../conf/train.yaml') as file:
    ...
    #stockData = getFullData(allStocks, interval, dataDir)
    stockData = getStockDataFromDb(allStocks, dbInfoFile, marketType = marketType)
